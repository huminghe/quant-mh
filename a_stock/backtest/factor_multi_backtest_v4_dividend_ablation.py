...
本脚本在v3已有7因子（reversal/ep_sector/ocf/roe/profit_stability/turnover/sue）
基础上加入dividend，共8个因子，255种非空子集全遍历，方法论与v3完全一致：
1. 截面相关性检验：8个因子两两截面相关性，排除高度冗余（|相关性|>0.5）的对
2. 全子集消融：255种非空子集等权合成，Top30月度调仓，全样本/IS/OOS三段评估

只测中证500（沿用v3的INDEX_KEY，与SUE仅中证500通过初筛的既有设定一致，
便于直接对比v3的"V2基线/去roe加sue"等参照行）。

用法：
  cd a_stock/backtest
  python factor_multi_backtest_v4_dividend_ablation.py
"""
...
from factor_multi_backtest_v2 import (  # noqa: E402
    get_industry_map, compute_reversal, compute_ep_sector, compute_ocf, compute_roe,
...
from factor_ic_liquidity import (  # noqa: E402
    load_field_panel, load_circ_mv_panel, compute_turnover_daily,
)
from factor_ic_sue import compute_sue_pit  # noqa: E402
from factor_ic_dividend import load_dividend_panel  # noqa: E402

OUTPUT_DIR = pathlib.Path(__file__).parent / "results" / "factor_multi_backtest_v4_dividend_ablation"
...
INDEX_KEY  = "hs500"   # 与v3一致：SUE仅中证500通过初筛，本消融只在中证500上跑
MEMBERS_FILE = DATA_DIR / "hs500_members.parquet"
INDEX_NAME = "中证500"

FACTOR_NAMES = ["reversal", "ep_sector", "ocf", "roe", "profit_stability", "turnover", "sue", "dividend"]
...
def compute_month_factors(
    ...
    div_cols = [c for c in available if c in dividend_panel.columns]
    div_pit = dividend_panel.loc[:month_end]
    if div_cols and not div_pit.empty:
        raw["dividend"] = div_pit.iloc[-1][div_cols].dropna()  # 正向：股息率越高越好
    else:
        raw["dividend"] = pd.Series(dtype=float)
...
def precompute_monthly_data(
    ...
    for i, month_end in enumerate(monthly_last[:-1]):
        ...
        factors = compute_month_factors(close_panel, turnover_panel, dividend_panel, available, month_end, industry_map)
        if not factors:
            continue
...
def main():
    ...
    print("预计算月度截面因子（8个因子：反转/行业内EP/OCF/ROE/盈利稳定性/换手率/SUE/股息率）...")
    entries = precompute_monthly_data(close_panel, turnover_panel, dividend_panel, industry_map)
    ...
    if high_corr_pairs:
        ...
        print("\n无高相关性对（|corr|均<=0.5），8个因子两两基本独立。")

    # ── 阶段2：全子集消融 ──
    print(f"\n{'='*60}")
    print("阶段2：全子集消融（255种非空子集，等权合成，Top30）")
    print(f"{'='*60}")
    ...
    # ── 增量贡献判定：含dividend vs 不含dividend ──
    with_div = result_df[result_df["subset"].apply(lambda s: "dividend" in s.split("+"))]
    without_div = result_df[~result_df["subset"].apply(lambda s: "dividend" in s.split("+"))]
    print(f"\n=== dividend 增量贡献判定 ===")
    print(f"含dividend的{len(with_div)}个子集：全样本超额均值={with_div['全样本超额年化'].mean()*100:+.2f}%  "
          f"OOS超额均值={with_div['OOS超额年化'].mean()*100:+.2f}%")
    print(f"不含dividend的{len(without_div)}个子集：全样本超额均值={without_div['全样本超额年化'].mean()*100:+.2f}%  "
          f"OOS超额均值={without_div['OOS超额年化'].mean()*100:+.2f}%")
    ...
    print(f"\n去roe+加sue+加dividend（v3最优组合再加dividend）：")
    target = {"reversal", "ep_sector", "ocf", "profit_stability", "sue", "dividend"}
    cmp_row = result_df[result_df["subset"].apply(lambda s: set(s.split("+")) == target)]
    if not cmp_row.empty:
        row = cmp_row.iloc[0]
        print(f"    全样本超额年化={row['全样本超额年化']*100:+.1f}%  "
              f"IS={row['IS超额年化']*100:+.1f}%  OOS={row['OOS超额年化']*100:+.1f}%  "
              f"夏普={row['全样本夏普']:.3f}  最大回撤={row['全样本最大回撤']*100:.1f}%")
