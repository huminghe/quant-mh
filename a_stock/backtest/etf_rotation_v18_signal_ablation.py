"""
第十三轮：margin_balance并入候选池后的4信号全子集消融实验

背景：v17对3个新方向做IC检验，仅margin_balance（两融余额行业确认）通过
初筛（IC=+0.0414，年度同向占比87.5%，与动量/crowding/vol_ratio/flow
截面相关性均<0.05，独立）。moneyflow_ratio、rate_beta均未达阈值排除。

候选池现扩大为4个：crowding、vol_ratio、flow、margin_balance。
本脚本对全部15种非空子集做等权排名集成消融，统一用v16验证最优的
"连续打折"叠加方式，找出真正最优的信号子集，并对最优子集做滚动2年
窗口稳健性检验（沿用v16已验证的实现）。

margin_balance仅覆盖27只有申万行业映射的行业ETF，与flow/vol_ratio的
覆盖处理方式一致：信号缺失的标的（宽基/QDII）不参与该信号的boost，
combo_rank对多信号取均值时按天动态跳过缺失部分。
"""
...
def main():
    ...
    all_names = ["crowding", "vol_ratio", "flow", "margin_balance"]
    subsets = []
    ...
    # 对比第十二轮已确认的三信号方案（crowding+vol_ratio+flow）
    old_best_label = "crowding+vol_ratio+flow"
    if old_best_label in df.index:
        old_sharpe = df.loc[old_best_label, "夏普"]
        print(f"对照：第十二轮三信号方案「{old_best_label}」夏普={old_sharpe:.3f}，"
              f"新方案Δ={best_sharpe - old_sharpe:+.3f}")
