"""
MAX因子（彩票偏好）组合回测（指数增强候选，已通过IC初筛+冗余性检验）

背景：`factor_ic_max.py` 验证MAX(N)因子（过去21日内最大N日收益均值，负向使用）
月度截面Rank IC：沪深300 MAX(1) IC均值+0.0495（ICIR 0.262，年度同向63.6%），
中证500 MAX(1) IC均值+0.0569（ICIR 0.380，年度同向90.9%），均通过项目既定
初筛阈值。冗余性检验（与已证伪的ST突显效应因子、21日简单反转因子的截面
相关性均为中等水平0.42-0.45，剔除两者排序成分后残差IC均不降反升）确认
MAX因子不是二者的马甲，有独立增量。

风险提示（本脚本执行前已知）：MAX因子构造方式（过去1个月日收益极值信息）
与已证伪的ST因子高度相似，ST证伪的关键原因是组合层面月度截面排序剧烈
波动导致月换手率77-78%，成本侵蚀了IC阶段的微弱优势。本脚本用于验证MAX
因子是否重演同样的证伪结局。

本脚本参照 factor_salience_backtest.py（T+1建仓/涨跌停检查/实际换手成本
/容量估算/年度拆分/反向"规避买入"检验）的完整方法论。

因子定义：signal = -MAX1_t（MAX1见factor_ic_max.compute_max_factor，
top_n=1，过去21个交易日滚动窗口内最大单日收益，PIT，不依赖未来数据）。
使用MAX(1)而非MAX(5)：IC初筛阶段两者表现接近，MAX(1)是Bali原始定义，
优先验证。

组合构建（与factor_salience_backtest.py完全一致的方法论）：
- 股票池：沪深300 + 中证500（两个指数都通过IC初筛）
- 月度调仓，Top20%等权做多，T+1建仓，涨停剔除，实际换手成本
  （ROUND_TRIP_COST=0.00164，trading-standards.md唯一权威成本口径）
- 基准：对应指数点位；成分股等权universe作诊断参考
- 回测区间：2016-01 ~ 2026-06（与IC验证期一致）
- 反向信号"规避买入"检验：MAX(1)最高（近期出现极端暴涨日，理论上被
  高估）组是否显著跑输universe

用法：
  cd a_stock/backtest
  python factor_max_backtest.py --index hs300
  python factor_max_backtest.py --index hs500
"""
...
from factor_ic_max import compute_max_factor, MAX_WINDOW  # noqa: E402

OUTPUT_DIR = pathlib.Path(__file__).parent / "results" / "factor_max_backtest"
...
def compute_cross_section_signal(close_panel: pd.DataFrame, codes: list[str],
    ...
    """
    signal = -MAX1_t（MAX1见factor_ic_max.compute_max_factor，top_n=1）。
    ...
    max1 = compute_max_factor(window, TOP_N)
    return -max1  # 负向使用：高MAX1（近期极端暴涨日）-> 低信号，回避


# ── 绩效统计（同factor_salience_backtest.py口径） ──────────
...
def run_avoid_buy_check(close_panel: pd.DataFrame, trade_days: pd.DatetimeIndex,
    ...
    """
    反向检验：MAX(1)最高（近期极端暴涨日，理论上被高估）组是否显著跑输universe。
    ...
    for i in range(len(monthly_last) - 1):
        ...
        n_select = max(1, int(len(signal) * top_pct))
        worst_group = signal.nsmallest(n_select).index.tolist()  # signal最低=MAX1最高=最该回避
...
def summarize_performance(ret_df: pd.DataFrame, index_name: str, index_code: str) -> None:
    ...
    print(f"\n=== 组合回测绩效（{index_name}，月度调仓，MAX(1)因子Top信号做多，等权） ===")
...
def summarize_avoid_buy(avoid_df: pd.DataFrame) -> None:
    print("\n=== 反向信号检验：MAX(1)最高（近期极端暴涨日）组“规避买入”效应 ===")
...
def main():
    parser = argparse.ArgumentParser(description="MAX(1)因子组合回测")
