...
因子构造：rqye月末快照的月度环比变化率 pct_change = rqye(t)/rqye(t-1) - 1
用环比变化率而非绝对变化量，天然按个股自身基线归一化，避免大盘股融券
余额绝对值天然更大导致的规模偏差，不需要额外联表流通市值（KISS）。

假设方向：融券余额环比大幅上升 → 做空/看空力量增强 → 预期下月收益为负
（不预设方向，IC符号由数据检验决定，不主观锚定"上升即看空"）。

验证方式：每月末截面 Spearman Rank IC（因子值 vs 下月收益率），沿用项目
既有factor_ic_*.py方法论。入选阈值：|IC均值|>=0.03 且年度同向占比>=60%。

用法：
  cd a_stock/backtest
  python factor_ic_short_balance.py               # 默认跑全部
  python factor_ic_short_balance.py --index hs300
"""
...
from fetch_short_balance import load_short_balance_panel

OUTPUT_DIR = pathlib.Path(__file__).parent / "results" / "factor_ic_short_balance"
...
def compute_pct_change_factor(balance_panel: pd.DataFrame) -> pd.DataFrame:
    """
    rqye月末快照 -> 环比变化率面板。
    rqye=0会导致pct_change产生inf，转为NaN不参与截面计算。
    """
    pct = balance_panel.pct_change()
...
def compute_monthly_ic(
    ...
    for i, month_end in enumerate(monthly_last[:-1]):
        ...
        # 因子面板用月末快照日期索引，找<=month_end的最近一行
        factor_dates = factor_panel.index[factor_panel.index <= month_end]
        if len(factor_dates) == 0:
            continue
        factor_row = factor_panel.loc[factor_dates[-1]]
        factor = factor_row[[c for c in available if c in factor_row.index]].dropna()
...
# ── 统计汇总（同factor_ic_fund_holding.py） ──────────────────
...
def main():
    parser = argparse.ArgumentParser(description="融券余额环比变化因子截面IC验证")
    ...
    print("加载融券余额数据...")
    balance_panel = load_short_balance_panel()
    print(f"面板大小：{balance_panel.shape}（{balance_panel.index.min()} ~ {balance_panel.index.max()}）")
    factor_panel = compute_pct_change_factor(balance_panel)
    ...
    coverage = len(set(balance_panel.columns) & all_codes) / len(all_codes) if all_codes else 0
    print(f"成分股覆盖率（曾有融券余额记录）：{coverage:.1%}\n")
    ...
    for key in index_keys:
        ...
        ic_df = compute_monthly_ic(close_panel, members_file, factor_panel)
        if ic_df.empty:
            print("  无有效数据")
            continue
        ...
        plot_ic(ic_df, out_dir, f"{name} 融券余额环比变化因子")
