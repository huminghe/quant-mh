...
只披露前十大重仓股，半年报/年报（Q2/Q4）才披露接近完整持仓。全量下载
660只基金实测：Q2/Q4的stk_mkv_ratio加总中位数99.99%（基本是完整持仓，
不是"接近完整"），Q1/Q3（用同接口的ETF池`fund_portfolio_cache`实测）
中位数约51.9%（前十大重仓集中度更高的基金会更高，含大量50-60%区间）。
本脚本只拉取Q2/Q4记录，Q1/Q3记录不落盘，避免下游因子计算把"前十大"和
"完整持仓"两种口径混用。
...
数据特性（已实测确认，ETF池`fund_portfolio_cache`全量数据）：
- fund_portfolio 单次全历史请求（2016至今）不会像fund_adj那样静默截断
- Q1/Q3（一季报/三季报）中位数10条（前十大重仓，约50.7%恰好=10条，
  范围1-20条），Q2/Q4（半年报/年报）中位数75.5条（范围1-2032条，规模
  越大持仓越分散）——本脚本按ann_date对应的end_date月份判断季度，只
  保留Q2(6月)/Q4(12月)记录
