"""
计算每只候选ETF的申万一级行业暴露分布与集中度，用于替代人工挑选行业的分层候选池方案。

数据来源：
- fund_portfolio_cache/{ts_code}.parquet（fetch_etf_portfolio_holdings.py 拉取的季度持仓明细）
- stock_sw_industry.parquet（个股→申万一级行业映射，31个行业）
...
- 未匹配到申万一级行业的持仓（可转债/港股通/现金等）计入"未分类"，不进入任何行业分子，
...
def classify_etf(ts_code: str, sw_map: pd.DataFrame) -> dict:
    ...
    snap = snap.merge(sw_map, left_on="symbol", right_on="ts_code", how="left", suffixes=("", "_sw"))

    total_ratio = snap["stk_mkv_ratio"].sum()
    matched_ratio = snap.loc[snap["sw_industry"].notna(), "stk_mkv_ratio"].sum()
    match_rate = matched_ratio / total_ratio if total_ratio > 0 else 0.0

    exposure = snap.groupby("sw_industry")["stk_mkv_ratio"].sum() / 100.0
...
def main():
    ...
    sw_map = pd.read_parquet(SW_INDUSTRY_PATH)

    results = []
    exposure_rows = []
    low_match_warnings = []
    for i, code in enumerate(candidates, 1):
        r = classify_etf(code, sw_map)
